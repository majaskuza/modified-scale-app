...
from automatic_spreadsheet import add_recording_entry, save_recording_df, load_animal_data_dataframe
#modify this later to remote path
local_file_path = "/Users/majaskuza/desktop/local_weight_record.xlsx"


st.title("Animal Weighting and Running Controller")

#new line added by maja
experimenter_type = st.radio("Select Experimenter Type:", ("Akrami", "Not Akrami"))
st.write(f"You selected: {experimenter_type}")

   
if experimenter_type == "Akrami":
    st.write("You have selected Akrami-specific logic.")

    excel_file_path = "/Users/majaskuza/desktop/local_weight_record.xlsx"
    try:
    ...
    if not st.session_state.scale_conn:
        ...
        if st.button('Connect with the scale'):
            try:
                ...
                if scale_ctx.conn == 1:
                    ...
                    st.write('Please check/retry scale connection')
            except Exception as e:
                st.error(f'Connection error: {e}')
    
    # Read RFID from keyboard input (UID scan devices)
    RFID_text = st.text_input(label='You can type or scan the RFID here', value='', key="rfid_text_input")

    def clear_text():
        st.session_state["rfid_text_input"] = ""

    if len(RFID_text) > 14:
        RFID_text = RFID_text[0:15]  # Only read first 15 digits
        
        # Extract animal information from the Excel DataFrame
        animal_info = df[df['RFID'] == RFID_text]

        if not animal_info.empty:
            subjid = animal_info['AnimalID'].values[0]
            pyratid = animal_info['Pyratid'].values[0]
            cageid = animal_info['CageID'].values[0]
            expgroup = animal_info['ExpGroup'].values[0]
            animal_owner = animal_info['Owner'].values[0]
            d = {'subjid': [subjid], 'pyratid': [pyratid], 'cageid': [cageid], 'expgroup': [expgroup], 'owner': [animal_owner]}
            df_display = pd.DataFrame(data=d)
            st.table(df_display)
        ...
        if scale_ctx.conn == 1:
            scale_ctx.serial_port_clx.flushInput()  
            ...
            if rigid > 0:
                with col1_run:
                    if st.button('RUN', on_click=clear_text):
                        st.write('Please put the animal on the scale and wait for a reading')
                        weight_reading = scale_ctx.read_scale_based_on_setting()
                        st.write(f'Weight is {weight_reading:.2f}g')
                        # Logic for running the animal and recording the weight
                ...
                with col2_weight:
                    if st.button('WEIGHT ONLY', on_click=clear_text):
                        ...
                        weight_reading = scale_ctx.read_scale_based_on_setting()
                        st.write(f'Weight is {weight_reading:.2f}g')
                        # Submit animal weight logic
                        if verify_weight(dbc, subjid, weight_reading):  # Verify weight logic
                            display_success = f'Weight for {subjid} saved.'
                            st.session_state.allow_rfid_read = True
                            st.session_state.current_subjid = 0
                            st.session_state.current_rigid = 0
                        else:
    ...
    st.write("You have selected Not Akrami.")
